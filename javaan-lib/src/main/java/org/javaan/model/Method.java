...
public class Method extends NamedObjectBase {
	...
	public static Method create(Type type, org.apache.bcel.classfile.Method javaMethod) {
		...
		List<String> parametersTypes = SignatureUtil.createClassSignatures(javaMethod.getArgumentTypes());
		List<String> annotationTypes = SignatureUtil.createClassSignatures(javaMethod.getAnnotationEntries());
		return new Method(name, type, signature, methodName, isAbstract, returnType, parametersTypes, annotationTypes, javaMethod);
	}
	...
	public static Method create(Type type, java.lang.reflect.Method method) {
		...
		List<String> parameterTypes = SignatureUtil.createClassSignatures(method.getParameterTypes());
		List<String> annotationTypes = SignatureUtil.createClassSignatures(method.getAnnotations());
		return new Method(name, type, signature, methodName, isAbstract, returnType, parameterTypes, annotationTypes);
	}
	...
	public static Method create(Type type, Constructor<?> constructor) {
		...
		List<String> parameterTypes = SignatureUtil.createClassSignatures(constructor.getParameterTypes());
		List<String> annotationTypes = SignatureUtil.createClassSignatures(constructor.getAnnotations());
		return new Method(name, type, signature, SignatureUtil.CONSTRUCTOR_SIGNATURE, isAbstract, returnType, parameterTypes, annotationTypes);
	}
