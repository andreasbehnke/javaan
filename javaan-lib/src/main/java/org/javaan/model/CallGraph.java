...
public class CallGraph {
	...
	private void addUsageOfType(Method caller, Method callee) {
		...
		// check whether dependencies of class hierarchy should be resolved and
		// if caller and callee are contained in class hierarchy
	...
	public Set<Method> getCallee(Method caller) {
	...
	public void traverseCallee(Method caller, GraphVisitor<Method, String> calleeVisitor) {
	...
	public Set<Method> getLeafCallee(Method caller) {
	...
	public Set<Package> getLeafUsedPackages(Package using) {
		...
		return getPackageOfTypes(usedTypes, using);
	}
	...
	public Set<Package> getLeafUsingPackages(Package used) {
		...
		return getPackageOfTypes(usingTypes, used);
	}

	private Set<Package> getPackageOfTypes(Set<Type> types, Package ignorePackage) {
		Set<Package> packages = new HashSet<>();
		for (Type type : types) {
			Package packageOfType = classContext.getPackageOfType(type);
			if (!ignorePackage.equals(packageOfType)) {
				packages.add(packageOfType);
			}
		}
		return packages;
	}
