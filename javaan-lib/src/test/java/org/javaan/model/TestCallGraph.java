...
public class TestCallGraph {
	...
	public void testMultipleUsageOfType() {
		...
		assertTrue(d.getCallee().contains(B_METHODD));
		assertTrue(d.getCallee().contains(B_METHODD1));
	...
	public void testGetCallee() {
		...
		Set<Method> callee = callGraph.getCallee(A_METHODA);
		assertNotNull(callee);
		assertEquals(2, callee.size());
		assertTrue(callee.contains(A_METHODB));
		assertTrue(callee.contains(A_METHODC));
		callee = callGraph.getCallee(A_METHODB);
		assertNotNull(callee);
		assertEquals(0, callee.size());
		callee = callGraph.getCallee(A_METHODC);
		assertNotNull(callee);
		assertEquals(0, callee.size());
	}
	...
	@Test
	public void testTraverseCallee() {
		CallGraph callGraph = createCallGraph();
		callGraph.addCall(A_METHODA, A_METHODB);
		callGraph.addCall(A_METHODA, A_METHODC);
		callGraph.addCall(A_METHODC, B_METHODD);
		...
		callGraph.traverseCallee(A_METHODA, visitor);
	...
	@Test
	public void testGetLeafCallee() {
		CallGraph callGraph = createCallGraph();
		callGraph.addCall(A_METHODA, A_METHODB);
		callGraph.addCall(A_METHODA, A_METHODC);
		callGraph.addCall(A_METHODC, B_METHODD);

		Set<Method> leafCallee = callGraph.getLeafCallee(A_METHODA);
		assertNotNull(leafCallee);
		assertEquals(2, leafCallee.size());
		assertTrue(leafCallee.contains(A_METHODB));
		assertTrue(leafCallee.contains(B_METHODD));

		leafCallee = callGraph.getLeafCallee(B_METHODD);
		assertNotNull(leafCallee);
		assertEquals(0, leafCallee.size());
	}
	...
	public void testGetDependencyCycles() {
		...
		callGraph.addCall(B_METHODD, A_METHODA);// second cycle A --> B --> A
